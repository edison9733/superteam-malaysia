...
export default function CTA({ content }) {
  ...
  const title = content?.join_title || 'Ready to Build the Future?';
  const subtitle = content?.join_subtitle || 'Join Superteam Malaysia and connect with the most ambitious builders in the Solana ecosystem.';
  const telegramUrl = content?.telegram_url || 'https://linktr.ee/SuperteamMY';
  const discordUrl = content?.discord_url || 'https://discord.gg/rrUMX92p';
  const twitterUrl = content?.twitter_url || 'https://x.com/SuperteamMY';

  const socials = [
    { name: 'Community', url: telegramUrl, icon: '🔗' },
    { name: 'Discord', url: discordUrl, icon: '💬' },
    { name: 'Twitter / X', url: twitterUrl, icon: '𝕏' },
  ];
